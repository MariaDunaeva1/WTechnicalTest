...
def test_answer_strips_whitespace():
    answer = Answer(question_id="q1", text="  I like to plan everything.  ")
    assert answer.text == "I like to plan everything."


def test_assessment_request_requires_exactly_five_answers():
    answers = [Answer(question_id=f"q{i}", text="answer") for i in range(4)]
    with pytest.raises(ValidationError):
        AssessmentRequest(answers=answers)


def test_assessment_request_accepts_five_answers():
    answers = [Answer(question_id=f"q{i}", text="answer") for i in range(5)]
...
def test_dimension_score_rejects_out_of_range(score):
    with pytest.raises(ValidationError):
        DimensionScore(score=score, rationale="reason")


def test_dimension_score_accepts_valid_range():
    score = DimensionScore(score=3, rationale="reason")
    assert score.score == 3


def test_big_five_profile_requires_all_dimensions():
    valid_dim = {"score": 3, "rationale": "reason"}
    with pytest.raises(ValidationError):
        BigFiveProfile(
            openness=valid_dim,
